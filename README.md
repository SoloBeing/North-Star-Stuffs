...
- **10 form templates** — NSP scholarship, PAN Form 93, ration card, pension
  ...
  LPG subsidy, RTI. 100 fields, every one with a hand-written Hindi *and*
  English explanation.
...
- **The real government form, filled** — for PAN Form 93, answers are stamped
  into the department's own PDF, one character per box: **17 of its 24 numbered
  items**, including the parents' section, the residential-status declaration
  and the verification block. Every cell coordinate was read out of the form's
  vector strokes, not measured by hand. The seven left empty are genuinely not
  ours to fill — the AO code the PAN centre looks up, the passport number only
  non-residents need, the representative-assessee block — and each one is named
  and explained to the citizen rather than left as a silent blank.
